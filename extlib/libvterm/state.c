...
static int on_text(const char bytes[], size_t len, void *user) {
	...
	(*encoding->enc->decode)(encoding->enc, encoding->data, codepoints, &npoints, state->gsingle_set ? 1 : len, bytes, &eaten, len);
...
static void set_dec_mode(VTermState *state, int num, int val) {
	switch (num) {
		...
		case 1002:
		case 1003:
			settermprop_int(state, VTERM_PROP_MOUSE, !val ? VTERM_PROP_MOUSE_NONE : (num == 1000) ? VTERM_PROP_MOUSE_CLICK :
		...
		case LEADER('?', 0x4a): // DECSED - Selective Erase in Display
			...
			switch (CSI_ARG(args[0])) {
				...
				case 0:
					...
					rect.start_row = state->pos.row + 1;
		...
		switch (command[0]) {
			case 'm': // Query SGR
			{
				...
				for (int argi = 0; argi < argc; argi++) {
					cur += snprintf(vt->tmpbuffer + cur, vt->tmpbuffer_len - cur, argi == argc - 1 ? "%ld" : CSI_ARG_HAS_MORE(args[argi]) ? "%ld:" :
                                                                                                                                            "%ld;",
									CSI_ARG(args[argi]));
					if (cur >= vt->tmpbuffer_len)
						return;
